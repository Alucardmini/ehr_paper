...
def get_sample_datesets(data_path, task, sample_nums=1000, is_sampls=False):
    """
    以12小时为单位存储
...
def write_list_2_file(list, filepath):
    with open(filepath, 'w+') as f:
        f.write(str(list))


def save_train_data(train_x, train_y, dst_path):
    write_list_2_file(train_x, dst_path+'_x.txt')
    write_list_2_file(train_y, dst_path+'_y.txt')
...
if __name__ == "__main__":
    ...
    for task in tasks:
        texts, labels, sample_count = get_sample_datesets(data_path=data_path, task=task, sample_nums=1000, is_sampls=True)
        save_train_data(texts, labels, '../data/'+task)
